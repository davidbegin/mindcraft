...
test('defaults when no config file exists', () => {
    ...
    assert.equal(resolveVoiceName('test_bot'), autoVoiceName('test_bot'));
    assert.equal(resolveVoice('test_bot'), VOICE_POOL[autoVoiceName('test_bot')]);
    assert.equal(resolveVoice('test_bot'), resolveVoice('test_bot'));
});
...
test('saveVoicesConfig writes and resolveVoice honors priority', () => {
    saveVoicesConfig({
        ...
        bots: { test_bot: 'grimblewood', custom_bot: 'RawVoiceId123', empty: '  ' },
    });
    ...
    assert.deepEqual(written.bots, { test_bot: 'grimblewood', custom_bot: 'RawVoiceId123' });

    assert.equal(getElevenLabsModel(), 'eleven_turbo_v2_5');
    // Pinned beats default; pool names are case-insensitive.
    assert.equal(resolveVoiceName('test_bot'), 'Grimblewood');
    assert.equal(resolveVoice('test_bot'), VOICE_POOL.Grimblewood);
    ...
    assert.equal(resolveVoiceName('test_bot', 'inferno'), 'Inferno');
    assert.equal(resolveVoice('test_bot', 'Inferno'), VOICE_POOL.Inferno);
});

test('clearing default_voice restores auto assignment', () => {
    saveVoicesConfig({ bots: { test_bot: 'Grimblewood' } });
    const written = JSON.parse(readFileSync(configPath, 'utf8'));
